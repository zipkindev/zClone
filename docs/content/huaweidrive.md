---
title: "Huawei Drive"
description: "Zclone docs for Huawei Drive"
...
[snip]
Storage> huaweidrive
OAuth Client Id - leave blank to use zclone's default.
client_id> 
OAuth Client Secret - leave blank to use zclone's default.
client_secret>
Remote config
...
When you use zclone with Huawei Drive in its default configuration you
...
It is strongly recommended to use your own client ID as the default zclone ID is heavily used. If you have multiple services running, it is recommended to use a separate client ID for each service to avoid rate limiting issues.

Here is how to create your own Huawei Drive client ID for zclone:
...
Zclone uses the web application access method to connect to Huawei Drive. Before starting, you need to implement quick login authorization through Huawei Account Open Service. With user authorization credentials, zclone can call Huawei Cloud Drive Service APIs.
...
9. Provide the noted client ID and client secret to zclone during configuration.

### Scopes

Zclone uses the following OAuth2 scopes when accessing Huawei Drive:
...
The `drive.file` scope is included in the default set used by zclone.

Alternative scopes available (not used by zclone by default):
...
These restrictions are handled by zclone's encoding:
...
- Config:      client_id
- Env Var:     ZCLONE_HUAWEIDRIVE_CLIENT_ID
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_HUAWEIDRIVE_CLIENT_SECRET
- Type:        string
- Required:    false
...
- Config:      token
- Env Var:     ZCLONE_HUAWEIDRIVE_TOKEN
- Type:        string
- Required:    false
...
- Config:      auth_url
- Env Var:     ZCLONE_HUAWEIDRIVE_AUTH_URL
- Type:        string
- Required:    false
...
- Config:      token_url
- Env Var:     ZCLONE_HUAWEIDRIVE_TOKEN_URL
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_HUAWEIDRIVE_CLIENT_CREDENTIALS
- Type:        bool
- Default:     false
...
- Env Var:     ZCLONE_HUAWEIDRIVE_ROOT_FOLDER_ID
- Type:        string
- Required:    false
...
- Config:      chunk_size
- Env Var:     ZCLONE_HUAWEIDRIVE_CHUNK_SIZE
- Type:        SizeSuffix
- Default:     8Mi
...
- Config:      list_chunk
- Env Var:     ZCLONE_HUAWEIDRIVE_LIST_CHUNK
- Type:        int
- Default:     1000
...
- Env Var:     ZCLONE_HUAWEIDRIVE_UPLOAD_CUTOFF
- Type:        SizeSuffix
- Default:     20Mi
...
- Config:      encoding
- Env Var:     ZCLONE_HUAWEIDRIVE_ENCODING
- Type:        Encoding
...
- Config:      description
- Env Var:     ZCLONE_HUAWEIDRIVE_DESCRIPTION
- Type:        string
- Required:    false
...
Huawei Drive has some rate limiting which may cause zclone to slow down transfers to avoid hitting the limits.
...
Huawei Drive imposes API rate limits. Zclone automatically handles these by backing off when limits are hit.

### OAuth2 token expiry

- Access tokens expire after 1 hour
- Refresh tokens expire after 180 days  
- Zclone will automatically refresh tokens as needed
...
- Huawei Drive has a special "applicationData" folder for app-specific data (not accessible via zclone's default configuration)
...
- `429 Too Many Requests`: API rate limit exceeded, zclone will automatically retry
...
- Use `zclone sync` rather than `zclone copy` to maintain exact directory structure
...
Huawei Drive supports the following zclone features:
...
- `ChangeNotify` - Poll for remote changes (used by `zclone mount` and `zclone rc vfs/poll-interval`)

## Integration with other services

Huawei Drive can be used as a backend for various zclone operations:

- Mount as a local filesystem using `zclone mount`
- Serve over HTTP using `zclone serve http`
- Serve over FTP using `zclone serve ftp`
- Serve over WebDAV using `zclone serve webdav`
