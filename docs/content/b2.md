...
You can use these with zclone too; you will need to use zclone version 1.43
or later.
...
names. Zclone hasn't changed its encoding as this could cause syncs to
re-transfer files. If you want zclone not to replace \ then see the
...
when zclone uploads a new version of a file it creates a [new version
...
flag or after creation using the [zclone backend lifecycle](#lifecycle)
command.
...
uploads, then you can use the `zclone cleanup remote:bucket` command
...
will be deleted, e.g. `zclone cleanup remote:bucket/path/to/stuff`.
...
expiry date then run `zclone backend cleanup b2:bucket -o max-age=1h`
...
```console
$ zclone -q ls b2:cleanup-test
        9 one.txt

$ zclone -q --b2-versions ls b2:cleanup-test
        9 one.txt
...
```console
$ zclone -q --b2-versions copy b2:cleanup-test/one-v2016-07-04-141003-000.txt /tmp
...
```console
$ zclone -q cleanup b2:cleanup-test
...
When using `--b2-versions` flag zclone is relying on the file name
...
zclone will show and act on older versions of files.  For example
...
Zclone supports generating file share links for private B2 buckets.
They can either be for a file for example:

```console
./zclone link B2:bucket/path/to/file.txt
...
- Env Var:     ZCLONE_B2_UPLOAD_CUTOFF
- Type:        SizeSuffix
- Default:     200Mi
...
- Env Var:     ZCLONE_B2_UPLOAD_CONCURRENCY
- Type:        int
- Default:     4
...
- Env Var:     ZCLONE_B2_DISABLE_CHECKSUM
- Type:        bool
- Default:     false
...
- Config:      download_url
- Env Var:     ZCLONE_B2_DOWNLOAD_URL
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_B2_DOWNLOAD_AUTH_DURATION
- Type:        Duration
- Default:     1w
...
- Env Var:     ZCLONE_B2_SSE_CUSTOMER_ALGORITHM
- Type:        string
- Required:    false
- Examples:
  - ""
...
- Env Var:     ZCLONE_B2_SSE_CUSTOMER_KEY
- Type:        string
- Required:    false
- Examples:
  - ""
...
- Env Var:     ZCLONE_B2_SSE_CUSTOMER_KEY_BASE64
- Type:        string
- Required:    false
- Examples:
  - ""
...
- Env Var:     ZCLONE_B2_SSE_CUSTOMER_KEY_MD5
- Type:        string
- Required:    false
- Examples:
  - ""
...
```console
zclone backend lifecycle remote: [options] [<arguments>+]
```
...
`zclone about` is not supported by the B2 backend. Backends without
