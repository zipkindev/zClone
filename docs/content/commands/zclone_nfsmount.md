...
Zclone nfsmount allows Linux, FreeBSD, macOS and Windows to
...
```console
zclone nfsmount remote:path/to/files /path/to/local/mount
```
...
```console
zclone nfsmount remote:path/to/files *
zclone nfsmount remote:path/to/files X:
zclone nfsmount remote:path/to/files C:\path\parent\mount
zclone nfsmount remote:path/to/files \\cloud\remote
```
...
To run `zclone nfsmount on Windows`, you will need to
...
during the implementation of zclone nfsmount for Windows.
...
```console
zclone nfsmount remote:path/to/files X: --network-mode
```
...
```console
zclone nfsmount remote:path/to/files X: --network-mode
zclone nfsmount remote:path/to/files X: --volname \\server\share
```
...
```console
zclone nfsmount remote:path/to/files \\cloud\remote
zclone nfsmount remote:path/to/files * --volname \\cloud\remote
```
...
## zclone nfsmount vs zclone sync/copy

File systems expect things to be 100% reliable, whereas cloud storage
systems are a long way from 100% reliable. The zclone sync/copy
commands cope with this with lots of retries.  However zclone nfsmount
...
When running zclone nfsmount as a systemd service, it is possible
to use Type=notify. In this case the service will enter the started state
after the mountpoint has been successfully set up.
Units having the zclone nfsmount service specified as a requirement
...
```
zclone nfsmount remote:path /path/to/mountpoint [flags]
```
...
```
      ...
      --daemon-timeout Duration                Time limit for zclone to respond to kernel (not supported on Windows) (default 10m0s)
  ...
  -o, --option stringArray                     Option for libfuse/WinFsp (repeat if required)
      ...
      --vfs-used-is-size zclone size           Use the zclone size algorithm for Used size
      --vfs-write-back Duration                Time to writeback files after last use when using cache (default 5s)
      --vfs-write-wait Duration                Time to wait for in-sequence write before giving error (default 1s)
      --volname string                         Set the volume name (supported on Windows and OSX only)
      --write-back-cache                       Makes kernel buffer writes before sending them to zclone (without this, writethrough caching is used) (not supported on Windows)
```
