...
```sh
zclone copyurl --urls myurls.csv remote:dir
```
...
If you can't get `zclone copyurl` to work then here are some things you can try:

- `--disable-http2` zclone will use HTTP2 if available - try disabling it
- `--bind 0.0.0.0` zclone will use IPv6 if available - try disabling it
...
```
zclone copyurl https://example.com dest:path [flags]
```
