...
Zclone mount allows Linux, FreeBSD, macOS and Windows to
...
```console
zclone mount remote:path/to/files /path/to/local/mount
```
...
```console
zclone mount remote:path/to/files *
zclone mount remote:path/to/files X:
zclone mount remote:path/to/files C:\path\parent\mount
zclone mount remote:path/to/files \\cloud\remote
```
...
To run `zclone mount on Windows`, you will need to
...
during the implementation of zclone mount for Windows.
...
```console
zclone mount remote:path/to/files X: --network-mode
```
...
```console
zclone mount remote:path/to/files X: --network-mode
zclone mount remote:path/to/files X: --volname \\server\share
```
...
```console
zclone mount remote:path/to/files \\cloud\remote
zclone mount remote:path/to/files * --volname \\cloud\remote
```
...
## zclone mount vs zclone sync/copy

File systems expect things to be 100% reliable, whereas cloud storage
systems are a long way from 100% reliable. The zclone sync/copy
commands cope with this with lots of retries.  However zclone mount
...
When running zclone mount as a systemd service, it is possible
to use Type=notify. In this case the service will enter the started state
after the mountpoint has been successfully set up.
Units having the zclone mount service specified as a requirement
...
```
zclone mount remote:path /path/to/mountpoint [flags]
```
...
```
      ...
      --daemon-timeout Duration                Time limit for zclone to respond to kernel (not supported on Windows) (default 0s)
