...
During the initial setup with `zclone config` you will specify the target
...
with the target `mydrive:private/backup`. Invoking `zclone mkdir backup:desktop`
...
Invoking `zclone mkdir backup:../desktop` is exactly the same as invoking
`zclone mkdir mydrive:private/backup/../desktop`.
...
```console
zclone copy /home/source remote:source
```
...
- Config:      description
- Env Var:     ZCLONE_ALIAS_DESCRIPTION
- Type:        string
- Required:    false
