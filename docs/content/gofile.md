---
title: "Gofile"
description: "Zclone docs for Gofile"
...
Note that if you wish to connect zclone to Gofile you will need a
premium account.
...
Gofile supports `zclone link` to make public links to files or
...
should be valid. Note that `zclone link --unlink` removes all the
...
directory you wish zclone to display.

You can do this with zclone

```console
$ zclone lsf -Fip --dirs-only remote:
...
To restrict zclone to the `Files` directory.
...
- Config:      access_token
- Env Var:     ZCLONE_GOFILE_ACCESS_TOKEN
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_GOFILE_ROOT_FOLDER_ID
- Type:        string
- Required:    false
...
- Config:      account_id
- Env Var:     ZCLONE_GOFILE_ACCOUNT_ID
- Type:        string
- Required:    false
...
- Config:      list_chunk
- Env Var:     ZCLONE_GOFILE_LIST_CHUNK
- Type:        int
- Default:     1000
...
- Config:      encoding
- Env Var:     ZCLONE_GOFILE_ENCODING
- Type:        Encoding
...
- Config:      description
- Env Var:     ZCLONE_GOFILE_DESCRIPTION
- Type:        string
- Required:    false
...
using a VFS based zclone command such as `zclone mount` you should
...
Zclone cannot sync that to a normal file system but it can be fixed
with the `zclone dedupe` command.
