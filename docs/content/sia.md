...
With zclone you can use it like any other remote filesystem or mount Sia folders
...
Before you can use zclone with Sia, you will need to have a running copy of
...
zclone interacts with Sia network by talking to the Sia daemon via [HTTP API](https://docs.sia.tech/)
...
several zclone and Sia-UI instances, you'll need to make a few more provisions:
...
- Set zclone backend option `api_password` taking it from above locations.

Notes:

1. If your wallet is locked, zclone cannot unlock it automatically.
...
2. If `siad` cannot find the `SIA_API_PASSWORD` variable or the `apipassword` file
   ...
   Remember this when you configure password in zclone.
...
- Upload a local directory to the Sia directory called *backup*
  ...
  ```console
  zclone copy /home/source mySia:backup
  ```
...
- Config:      api_password
- Env Var:     ZCLONE_SIA_API_PASSWORD
- Type:        string
- Required:    false
...
- Config:      description
- Env Var:     ZCLONE_SIA_DESCRIPTION
- Type:        string
- Required:    false
...
- `zclone about` not supported
- zclone can work only with *Siad* or *Sia-UI* at the moment,
  the **SkyNet daemon is not supported yet.**
- Sia does not allow control characters or symbols like question and pound
  signs in file names. zclone will transparently [encode](/overview/#encoding)
