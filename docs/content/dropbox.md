---
title: "Dropbox"
description: "Zclone docs for Dropbox"
...
Zclone supports Dropbox for business and Team Folders.
...
path, so `zclone lsd remote:/` will refer to the root and show you all
...
zclone which didn't support the v2 API and modified times, zclone will
...
There are 3 modes zclone can use for uploads.

#### --dropbox-batch-mode off

In this mode zclone will not use upload batching. This was the default
before zclone v1.55. It has the disadvantage that it is very likely to
...
When zclone receives these it has to wait for 15s or sometimes 300s
...
In this mode zclone will batch up uploads to the size specified by
...
Note that there may be a pause when quitting zclone while zclone
...
the caller. This means zclone can use a much bigger batch size (much
...
of small files, however zclone can't check the file got uploaded
properly using this mode.

If you are using this mode then using "zclone check" after the
...
When zclone downloads a exportable file, it chooses the format to
...
Zclone chooses the first format ID in the export formats list that
Dropbox supports for a given file. If no format in the list is
usable, zclone will choose the default format that Dropbox suggests.

Zclone will change the extension to correspond to the export format.
Here are some examples of how extensions are mapped:

| File type      | Filename in Dropbox | Filename in zclone |
|----------------|---------------------|--------------------|
| Paper          | mydoc.paper         | mydoc.html         |
| Paper template | mydoc.papert        | mydoc.papert.html  |
| other          | mydoc               | mydoc.html         |

*Importing* exportable files is not yet supported by zclone.

Here are the supported export extensions known by zclone. Note that
zclone does not currently support other formats not on this list,
...
- Config:      client_id
- Env Var:     ZCLONE_DROPBOX_CLIENT_ID
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_DROPBOX_CLIENT_SECRET
- Type:        string
- Required:    false
...
- Config:      auth_url
- Env Var:     ZCLONE_DROPBOX_AUTH_URL
- Type:        string
- Required:    false
...
- Config:      token_url
- Env Var:     ZCLONE_DROPBOX_TOKEN_URL
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_DROPBOX_CLIENT_CREDENTIALS
- Type:        bool
- Default:     false
...
- Config:      chunk_size
- Env Var:     ZCLONE_DROPBOX_CHUNK_SIZE
- Type:        SizeSuffix
- Default:     48Mi
...
- Config:      impersonate
- Env Var:     ZCLONE_DROPBOX_IMPERSONATE
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_DROPBOX_IMPERSONATE_ADMIN
- Type:        string
- Required:    false

#### --dropbox-shared-files

Instructs zclone to work on individual shared files.
...
- Config:      shared_files
- Env Var:     ZCLONE_DROPBOX_SHARED_FILES
- Type:        bool
- Default:     false

#### --dropbox-shared-folders

Instructs zclone to work on shared folders.
...
- Env Var:     ZCLONE_DROPBOX_SHARED_FOLDERS
- Type:        bool
- Default:     false

#### --dropbox-skip-shared-folders

Instructs zclone to skip all shared folders.
...
- Env Var:     ZCLONE_DROPBOX_SKIP_SHARED_FOLDERS
- Type:        bool
- Default:     false

#### --dropbox-skip-unowned-folders

Instructs zclone to skip shared folders not owned by the current user.
...
- Env Var:     ZCLONE_DROPBOX_SKIP_UNOWNED_FOLDERS
- Type:        bool
- Default:     false
...
- Env Var:     ZCLONE_DROPBOX_PACER_MIN_SLEEP
- Type:        Duration
- Default:     10ms
...
- Config:      encoding
- Env Var:     ZCLONE_DROPBOX_ENCODING
- Type:        Encoding
...
- Env Var:     ZCLONE_DROPBOX_ROOT_NAMESPACE
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_DROPBOX_EXPORT_FORMATS
- Type:        CommaSepList
- Default:     html,md
...
- Config:      skip_exports
- Env Var:     ZCLONE_DROPBOX_SKIP_EXPORTS
- Type:        bool
- Default:     false
...
- Env Var:     ZCLONE_DROPBOX_SHOW_ALL_EXPORTS
- Type:        bool
- Default:     false
...
For full info see [the main docs](//dropbox/#batch-mode)
...
- Config:      batch_mode
- Env Var:     ZCLONE_DROPBOX_BATCH_MODE
- Type:        string
- Default:     "sync"
...
- Config:      batch_size
- Env Var:     ZCLONE_DROPBOX_BATCH_SIZE
- Type:        int
- Default:     0
...
- Env Var:     ZCLONE_DROPBOX_BATCH_TIMEOUT
- Type:        Duration
- Default:     0s
...
- Config:      description
- Env Var:     ZCLONE_DROPBOX_DESCRIPTION
- Type:        string
- Required:    false
...
of this document](https://www.dropbox.com/en/help/145).  Zclone will
...
If you have more than 10,000 files in a directory then `zclone purge
dropbox:dir` will return the error `Failed to purge: There are too
many files involved in this operation`.  As a work-around do an
`zclone delete dropbox:dir` followed by an `zclone rmdir dropbox:dir`.

When using `zclone link` you'll need to set `--expire` if using a
...
When you use zclone with Dropbox in its default configuration you are using
zclone's App ID. This is shared between all the zclone users.

Here is how to create your own Dropbox App ID for zclone:
...
4. Name your App. The app name is global, so you can't use `zclone` for example
...
values in zclone config to add a new remote or edit an existing remote.
The `App key` setting corresponds to `client_id` in zclone config, the
