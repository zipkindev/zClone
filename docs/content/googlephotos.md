---
title: "Google Photos"
description: "Zclone docs for Google Photos"
versionIntroduced: "v1.49"
---

# Google Photos

The zclone backend for [Google Photos](https://www.google.com/photos/about/) is
a specialized backend for transferring photos and videos to and from
Google Photos.

**NB** The Google Photos API which zclone uses has quite a few
limitations, so please read the [limitations section](#limitations)
carefully to make sure it is suitable for your use.

**NB** From March 31, 2025 zclone can only download photos it
uploaded. This limitation is due to policy changes at Google. You may
need to run `zclone config reconnect remote:` to make zclone work
again after upgrading to zclone v1.70.
...
** See help for google photos backend at: //googlephotos/ **
...
y/n> n
Remote config
zclone's shared Google Photos client_id is being retired and will stop working during 2026.
Create your own to avoid interruption: //googlephotos/#making-your-own-client-id
...
```console
zclone lsd remote:album
```

Make a new album

```console
zclone mkdir remote:album/newAlbum
```

List the contents of an album

```console
zclone ls remote:album/newAlbum
```
...
```console
zclone sync --interactive /home/local/image remote:album/newAlbum
```
...
files you've uploaded for one zclone session only, becoming empty
again when you restart zclone. The use case for this would be if you
...
with a directory hierarchy in there then zclone will create albums
with the `/` character in them.  For example if you do

```console
zclone copy /path/to/images remote:album/images
```
...
Then zclone will create the following albums with the following files in
...
- Config:      client_id
- Env Var:     ZCLONE_GPHOTOS_CLIENT_ID
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_GPHOTOS_CLIENT_SECRET
- Type:        string
- Required:    false
...
- Config:      read_only
- Env Var:     ZCLONE_GPHOTOS_READ_ONLY
- Type:        bool
- Default:     false
...
- Config:      auth_url
- Env Var:     ZCLONE_GPHOTOS_AUTH_URL
- Type:        string
- Required:    false
...
- Config:      token_url
- Env Var:     ZCLONE_GPHOTOS_TOKEN_URL
- Type:        string
- Required:    false
...
- Env Var:     ZCLONE_GPHOTOS_CLIENT_CREDENTIALS
- Type:        bool
- Default:     false
...
- Config:      read_size
- Env Var:     ZCLONE_GPHOTOS_READ_SIZE
- Type:        bool
- Default:     false
...
- Config:      start_year
- Env Var:     ZCLONE_GPHOTOS_START_YEAR
- Type:        int
- Default:     2000
...
- Env Var:     ZCLONE_GPHOTOS_INCLUDE_ARCHIVED
- Type:        bool
- Default:     false
...
- Config:      encoding
- Env Var:     ZCLONE_GPHOTOS_ENCODING
- Type:        Encoding
...
- Config:      batch_mode
- Env Var:     ZCLONE_GPHOTOS_BATCH_MODE
- Type:        string
- Default:     "sync"
...
- Config:      batch_size
- Env Var:     ZCLONE_GPHOTOS_BATCH_SIZE
- Type:        int
- Default:     0
...
- Env Var:     ZCLONE_GPHOTOS_BATCH_TIMEOUT
- Type:        Duration
- Default:     0s
...
- Config:      description
- Env Var:     ZCLONE_GPHOTOS_DESCRIPTION
- Type:        string
- Required:    false
...
zclone will upload the file, then Google Photos will give an error
...
`zclone about` is not supported by the Google Photos backend. Backends without
...
See [zclone about](//commands/zclone_about/)
...
zclone to redownload original images.  You could use 'google takeout'
...
If a file name is duplicated in a directory then zclone will add the
...
filename from the first upload which may confuse zclone.  For example
...
This is not changeable by zclone and is not the modification date of
the media on local disk.  This means that zclone cannot use the dates
from Google Photos for syncing purposes.

### Size

The Google Photos API does not return the size of media.  This means
that when syncing to Google Photos, zclone can only do a file
existence check.
...
If you want to use the backend with `zclone mount` you may need to
...
Zclone can only upload files to albums it created. This is a
[limitation of the Google Photos API](https://developers.google.com/photos/library/guides/manage-albums).

Zclone can remove files it uploaded from albums it created only.

### Deleting files

Zclone can remove files from albums it created, but note that the
...
Zclone cannot delete files anywhere except under `album`.
...
When you use zclone with Google photos in its default configuration you
...
remotes created with `zclone config` will warn you if you leave the
client_id blank.

Please follow the steps in [the google drive docs](//drive/#making-your-own-client-id)
