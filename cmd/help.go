...
import (
	...
	"zclone/fs/filter"
	"zclone/fs/filter/filterflags"
	"zclone/fs/log/logflags"
...
var Root = &cobra.Command{
	Use:   fs.AppName,
	Short: "Show help for " + fs.AppName + " commands, flags and backends.",
	Long: fs.AppName + ` syncs files to and from cloud storage providers as well as
mounting them, listing them in lots of different ways.

Run ` + fs.AppName + ` help <command> for command-specific documentation.`,
	PersistentPostRun: func(cmd *cobra.Command, args []string) {
		fs.Debugf(fs.AppName, "Version %q finishing with parameters %q", fs.Version, os.Args)
		atexit.Run()
	},
...
// GeneratingDocs is set by gendocs to alter the format of the
...
var helpFlags = &cobra.Command{
	Use:   "flags [<filter>]",
	Short: "Show the global flags for zclone",
	Run: func(command *cobra.Command, args []string) {
		command.Flags()
		if GeneratingDocs {
...
func setupRootCommand(rootCmd *cobra.Command) {
	...
	cobra.AddTemplateFunc("showLocalFlags", func(cmd *cobra.Command) bool {
		// Don't show local flags (which are the global ones on the root) on "zclone" and
		// "zclone help" (which shows the global help)
		return cmd.CalledAs() != "zclone" && cmd.CalledAs() != ""
	})
...
Use "zclone [command] --help" for more information about a command.
Use "zclone help flags" for to see the global flags.
Use "zclone help backends" for a list of supported services.
`
...
title: "Global Flags"
description: "Zclone Global Flags"
# autogenerated - DO NOT EDIT
---

# Global Flags

This describes the global flags available to every zclone command
split into groups.
...
func showBackend(name string) {
	...
	for _, opts := range []fs.Options{standardOptions, advancedOptions} {
		...
		for _, opt := range opts {
			...
			fmt.Printf("%s\n\n", opt.Help)
			if opt.IsPassword {
				fmt.Printf("**NB** Input to this must be obscured - see [zclone obscure](/commands/zclone_obscure/).\n\n")
			}
