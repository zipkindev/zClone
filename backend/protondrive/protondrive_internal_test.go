...
func TestProtonDriveAppVersionFromZcloneVersion(t *testing.T) {
	testCases := []struct {
		name          string
		zcloneVersion string
		want          string
	}{
		{
			name:          "release",
			zcloneVersion: "v1.73.5",
			want:          "external-drive-zclone@1.73.5-stable",
		},
		{
			name:          "dev build",
			zcloneVersion: "v1.74.0-DEV",
			want:          "external-drive-zclone@1.74.0-dev",
		},
		{
			name:          "beta build with extra metadata",
			zcloneVersion: "v1.74.0-beta.9519.990f33f2a.fix-protondrive-sdk-2026",
			want:          "external-drive-zclone@1.74.0-beta.9519+990f33f2a.fix-protondrive-sdk-2026",
		},
		{
		...
		},
		{
			...
			zcloneVersion: "not-a-version",
			want:          "external-drive-zclone@1.0.0-stable",
		},
	}

	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			got := protonDriveAppVersionFromZcloneVersion(testCase.zcloneVersion)
