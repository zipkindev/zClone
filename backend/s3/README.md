...
It is quite easy to add a new S3 provider to zclone.

You'll then need to do add the following (optional tags are in [] and
do not get displayed in zclone config if empty):
...
- [Region]
  - Any regions your provider supports or the defaults (use `region: {}` for this)
    ...
    - The defaults (as seen in Zclone.yaml):
...
- `docs/content/s3.md`
  ...
  - Add a transcript of a trial `zclone config` session
...
- `docs/content/_index.md` - this is the home page of zclone.org
  - Add the provider and a link to the section you wrote in `docs/contents/s3.md`
...
Once you've written the code, test `zclone config` works to your
