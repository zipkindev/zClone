/*
This is a very simple test/demo program for libzclone's C interface
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void testRPC(char *method, char *in) {
    struct ZcloneRPCResult out = ZcloneRPC(method, in);
    printf("status: %d\n", out.Status);
    printf("output: %s\n", out.Output);
    ZcloneFreeString(out.Output);
}

// noop command
void testNoOp() {
    printf("test rc/noop\n");
    struct ZcloneRPCResult out = ZcloneRPC("rc/noop", "{"
            " \"p1\": [1,\"2\",null,4],"
            " \"p2\": { \"a\":1, \"b\":2 } "
            "}");
    printf("status: %d\n", out.Status);
...
void testError() {
    ...
    struct ZcloneRPCResult out = ZcloneRPC("rc/error",
            "{"
            " \"p1\": [1,\"2\",null,4],"
            " \"p2\": { \"a\":1, \"b\":2 } "
            "}");
...
int main(int argc, char** argv) {
    ...
    /* testRPC("config/setpath", "{\"path\":\"/tmp/zclone.conf\"}"); */
    /* testRPC("config/listremotes", "{}"); */
