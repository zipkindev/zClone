...
shims for accessing zclone from C and other languages.
...
[mount](/commands/zclone_mount/#installing-on-windows) documentation.
...
```console
go run bin/resource_windows.go -binary libzclone.dll -dir libzclone
```
...
- [ZcloneInitialize](https://pkg.go.dev/zclone/libzclone#ZcloneInitialize)
- [ZcloneFinalize](https://pkg.go.dev/zclone/libzclone#ZcloneFinalize)
- [ZcloneRPC](https://pkg.go.dev/zclone/libzclone#ZcloneRPC)
- [ZcloneFreeString](https://pkg.go.dev/zclone/libzclone#ZcloneFreeString)
...
not work on Windows. It is very possible to use `libzclone` from a C/C++
application on Windows, but there are some pitfalls that you can avoid by
following these guidelines:

- Build `libzclone` as shared library, and use run-time dynamic linking
  (see [linking](#linking)).
- Do not try to unload the library with `FreeLibrary`
  (see [unloading](#unloading)).
- Deallocate returned strings with API function `ZcloneFreeString`
  (see [memory management](#memory-management)).
- Define struct `ZcloneRPCResult`, instead of including `libzclone.h`
...
that references it, may easily break compatibility. When building the libzclone
...
`libzclone.dll` into your application it must stay loaded until your application
exits.

#### Memory management

The output string returned from `ZcloneRPC` is allocated within the `libzclone`
...
the API function `ZcloneFreeString`, which will call `free` from within the
`libzclone` library, using the same runtime that allocated it in the first
place.

#### Include file

Do not include `libzclone.h`. It contains some plain C, golang/cgo and GCC
...
The interface of libzclone is so simple, that all you need is to define the
small struct `ZcloneRPCResult`, from [libzclone.go](libzclone.go):
...
The API uses plain C strings (type `char*`, called "narrow" strings), and zclone
...
Conversion between UTF-8 encoded narrow strings used by zclone, and either ANSI
...
func main() {
  libzclone.Initialize()
    ...
    out, status: = libzclone.RPC("sync/copy", string(syncRequestJSON))
...
```console
gomobile bind -v -target=android -javapkg=org.zclone zclone/libzclone/gomobile
```

The command generates an Android library (`aar`) that can be imported
into an Android application project. Libzclone will be contained
within `libgojni.so` and loaded automatically.

```java
// imports
import org.zclone.gomobile.Gomobile;
import org.zclone.gomobile.ZcloneRPCResult;

// initialize zclone
Gomobile.zcloneInitialize();

// call RC method and log response.
ZcloneRPCResult response = Gomobile.zcloneRPC("core/version", "{}");
Log.i("zclone", "response status: " + response.getStatus());
Log.i("zclone", "output: " + response.getOutput());

// Clean up when finished.
Gomobile.zcloneFinalize();
```
...
API using zclone linked as a shared library with `ctypes`.
...
Rust bindings are available in the `libzclone` crate: <https://crates.io/crates/libzclone>

## PHP

The `php` subdirectory contains how to use the C library libzclone in php through
...
- Async jobs must currently be cancelled manually at the moment - ZcloneFinalize
  doesn't do it.
- This will use the zclone config system and zclone logging system.
